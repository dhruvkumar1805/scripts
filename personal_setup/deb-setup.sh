...
if [ "$EUID" -ne 0 ];
then
        echo -e "$red\nPlease run as root$txtrst\n"
        exit 1
fi

echo -e "$grn\nUpdating and installing packages. . .$txtrst\n"
sudo apt-get update -qq && sudo apt-get upgrade -y
sudo apt install -qq git gnupg flex bc bison neofetch build-essential zip curl zlib1g-dev gcc-multilib \
                        ...
                        libncurses5 lzop tmux libssl-dev patchelf apktool dos2unix git-lfs default-jdk \
